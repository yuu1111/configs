...
- `quality-check` は `quality.json` を読み、ruleの検証と preset / group の展開は実行時に engine の `./rule-ids` から読むため 3 engine を `peerDependencies` に持つ (下限だけの `>=` で示し、ruleを足した engine の minor へ合わせて上げる)
